...
// MetadataKey and MetadataAtespaceKey are the request metadata keys naming
// the target environment, shared with the env SDK.
const (
	MetadataKey         = "x-env-id"
	MetadataAtespaceKey = "x-env-atespace"
)

// Director resolves the environment named in a proxied call to the
// client connection the call is forwarded on. atespace is empty when the
// request does not name one.
type Director func(ctx context.Context, atespace, envID string) (*grpc.ClientConn, error)
...
// EnvID returns the environment id and atespace named in ctx's incoming
// request metadata; both are "" when absent.
func EnvID(ctx context.Context) (atespace, envID string) {
	md, _ := metadata.FromIncomingContext(ctx)
	if vals := md.Get(MetadataKey); len(vals) > 0 {
		envID = vals[0]
	}
	if vals := md.Get(MetadataAtespaceKey); len(vals) > 0 {
		atespace = vals[0]
	}
	return atespace, envID
}
...
func proxy(director Director, serverStream grpc.ServerStream) error {
	...
	conn, err := director(ctx, atespace, envID)
	if err != nil {
